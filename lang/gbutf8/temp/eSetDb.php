  ...
  <table width="100%" border="0" cellpadding="3" cellspacing="1" class="tableborder" id="setebma" style="display:none">
	...
	<tr>
	  ...
	  <td height="25" bgcolor="#FFFFFF"><input type="radio" name="ebmacklogin" value="0"<?=$ebak_ebma_cklogin==0?' checked':''?>>开启
          <input type="radio" name="ebmacklogin" value="1"<?=$ebak_ebma_cklogin==1?' checked':''?>>关闭 <font color="#666666">(开启后，进入phpmyadmin需要再次登录数据库，即：智安云备份验证+PMA本身双重验证)</font>
	  </td>
    </tr>
  </table>
	...
	?>
    <tr>
      <td height="30" colspan="2" bgcolor="#FFFFFF"><font color="#FF0000">首次安装智安云备份，请修改<strong>管理员设置</strong>里的默认<strong>管理密码</strong>和<strong>验证随机码</strong>。修改后需要重新登录后台。(至于默认<strong>用户名</strong>不强制，但官方推荐也修改)</font></td>
    </tr>
	<?php
	}
	?>
